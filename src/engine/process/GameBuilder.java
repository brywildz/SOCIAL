    ...
    public int getRandomIndexRelationShip(Personality personality, String type){
        ...
        if(type.equals("friends")){
            if(personality.getExtraversion().isHigh()){
                randomIndex = r.nextInt(20,50);
            }
            else if(personality.getExtraversion().isLow()){
                randomIndex = r.nextInt(0,10);
            }
            else{
                randomIndex = r.nextInt(5,19);
            }
        }
        else if(type.equals("work")){
            if(personality.getExtraversion().isHigh()){
                randomIndex = r.nextInt(5,15);
            }
            else if(personality.getAgreabilite().isHigh()){
                randomIndex = r.nextInt(5,10);
            }
            else if(personality.getAgreabilite().isLow()){
                randomIndex = r.nextInt(0,1);
            }
            else if(personality.getExtraversion().isLow()){
                randomIndex = r.nextInt(2,5);
            }
            else{
                randomIndex = r.nextInt(3,6);
            }
        }
        else if(type.equals("family")){
            randomIndex = r.nextInt(0,5);
        }
        return randomIndex;
    }
}
